...
>
<div>
<h1 className="text-xl font-bold tracking-wider" style={{ color: 'var(--text-primary)' }}>Override Audit Ledger</h1>
...
>
{isLoading ? (
<div className="text-sm py-12 text-center font-mono" style={{ color: 'var(--text-secondary)' }}>Loading ledger...</div>
) : entries.length === 0 ? (
<div
...
<tr
  ...
  style={{ backgroundColor: 'var(--bg-page)', color: 'var(--text-tertiary)', borderColor: 'var(--border-default)' }}
>
...
>
<td className="px-5 py-4 font-semibold" style={{ color: 'var(--text-secondary)' }}>{entry.id}</td>
<td className="px-5 py-4 font-semibold" style={{ color: 'var(--accent-primary)' }}>#{entry.claimId}</td>
...
<td className="px-5 py-4 text-xs" style={{ color: 'var(--text-secondary)' }}>{entry.authorizedBy}</td>
<td className="px-5 py-4 text-xs flex items-center gap-1.5 pt-5" style={{ color: 'var(--text-tertiary)' }}>
<Clock size={12} />
{format(new Date(entry.timestamp), 'yyyy-MM-dd HH:mm:ss')}
</td>
<td className="px-5 py-4 text-xs leading-relaxed max-w-sm whitespace-normal break-words font-sans" style={{ color: 'var(--text-secondary)' }}>
{entry.reason}
</td>
</tr>
))}
