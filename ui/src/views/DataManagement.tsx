...
interface SystemHealth {
  status: string;
  components?: Record<string, { status: string; threshold?: number; provider?: string }>;
}
    ...
    return (
      <div style={{ padding: 40, textAlign: 'center' }}>
        ...
        <p style={{ color: 'var(--text-secondary)', marginTop: 8 }}>
          Data management requires Tech Owner / Admin role.
        </p>
      </div>
    );
  }
  ...
  return (
    <div style={{ maxWidth: 720, display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div className="card" style={{ padding: 20 }}>
        ...
        {status?.lastSeededAt && (
          <p
            style={{ margin: '12px 0 0', fontSize: 12, color: 'var(--text-secondary)', fontFamily: 'var(--font-mono)' }}
          >
      ...
      <div className="card" style={{ padding: 20 }}>
        <h2 style={{ margin: '0 0 8px', fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>
          Seed Sample Data
        </h2>
      ...
      <div className="card" style={{ padding: 20 }}>
        ...
        <p style={{ margin: '0 0 16px', fontSize: 13, color: 'var(--text-secondary)' }}>
          Manually retrain the autoencoder on current ClaimProjections data. Requires at least 5 claim projections to
        ...
        {retrainModel.isSuccess && (
          ...
          >
            <CheckCircle size={14} />{' '}
            {
              ...
              (retrainModel.data as any)?.data?.message || 'Model retrained successfully.'
            }
          </p>
        )}
      ...
      <div className="card" style={{ padding: 20 }}>
        ...
        <p style={{ margin: '0 0 16px', fontSize: 13, color: 'var(--text-secondary)' }}>
          Download the full FHIR repository as a transaction Bundle. Use the upload section above to restore from a
          backup file.
        </p>
        <button
          onClick={() => {
      ...
      <div className="card" style={{ padding: 20 }}>
        <h2 style={{ margin: '0 0 8px', fontSize: 14, fontWeight: 600, color: 'var(--text-primary)' }}>
          Upload External Claim Data
        </h2>
        <p style={{ margin: '0 0 16px', fontSize: 13, color: 'var(--text-secondary)' }}>
          Upload a FHIR JSON file from your host system. Must be a valid FHIR R4 <strong>Claim</strong> resource or a{' '}
          <strong>Bundle</strong>.
        </p>
        <div
          onClick={() => fileInputRef.current?.click()}
          style={{
            border: `2px dashed ${uploadStatus === 'error' ? 'var(--color-danger-border)' : 'var(--border-strong)'}`,
            borderRadius: 8,
            padding: 32,
            textAlign: 'center',
            cursor: 'pointer',
        ...
        >
          ...
          <p style={{ margin: 0, fontSize: 14, color: 'var(--text-primary)', fontWeight: 500 }}>
            Click to upload a FHIR JSON file
          </p>
          <p style={{ margin: '4px 0 0', fontSize: 12, color: 'var(--text-tertiary)' }}>
            FHIR R4 JSON \u2014 Claim or Bundle resource (max 10 MB)
          </p>
          <input
            ref={fileInputRef}
            type="file"
            ...
            onChange={async (e) => {
              ...
              if (!file.name.endsWith('.json')) {
                ...
                setUploadMessage('Only JSON files are supported.');
                return;
              }
              if (file.size > 10 * 1024 * 1024) {
                setUploadStatus('error');
                setUploadMessage('File size exceeds the 10 MB limit.');
                return;
              }
              ...
              try {
                ...
                setUploadMessage(
                  `Successfully submitted${res.data?.projectionsCreated ? ` with ${res.data.projectionsCreated} projections` : ''}. FHIR status: ${res.data?.fhirStatus || 'OK'}`,
                );
                qc.invalidateQueries({ queryKey: ['claims'] });
                refetchStatus();
        ...
        {uploadStatus === 'success' && (
          ...
          >
            <CheckCircle size={14} /> {uploadMessage}
          </p>
        )}
        {uploadStatus === 'error' && (
          <p
          ...
          >
            <AlertTriangle size={14} /> {uploadMessage}
          </p>
        )}
      </div>
      ...
      >
        <h2 style={{ margin: '0 0 8px', fontSize: 14, fontWeight: 600, color: 'var(--color-danger)' }}>
          Danger Zone \u2014 Clear All Data
        </h2>
        <p style={{ margin: '0 0 16px', fontSize: 13, color: 'var(--text-secondary)' }}>
          This cannot be undone. All FHIR resources (Claims, ClaimResponses, Tasks, CommunicationRequests, Patients,
          DocumentReferences) and all projection tables (ClaimProjections, PatientProjections, ProviderProjections,
          ClinicalNotes) will be permanently deleted.
        </p>
        {!confirmClear ? (
          <button
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: 'var(--color-danger)' }}>
              Are you sure? This will delete all FHIR resources.
            </p>
            <div style={{ display: 'flex', gap: 8 }}>
              <button
                ...
                style={{
                  ...
                  fontWeight: 500,
                  cursor: 'pointer',
                  opacity: clearData.isPending ? 0.5 : 1,
                }}
              >
      ...
      <div className="card" style={{ padding: 20 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
            {[
              ...
              const color =
                status === 'healthy'
                  ? 'var(--color-success)'
                  : status === 'untrained' || status === 'unknown'
                    ? 'var(--color-warning)'
                    : 'var(--color-danger)';
              const extra =
                key === 'autoencoder' && c?.threshold
                  ? ` (threshold: ${c.threshold.toFixed(4)})`
                  : key === 'llm' && c?.provider
                    ? ` (${c.provider})`
                    : '';
              return (
                ...
                >
                  ...
                  <div style={{ minWidth: 0 }}>
                    <p style={{ margin: 0, fontSize: 11, fontWeight: 600, color: 'var(--text-primary)' }}>
                      {label}
                      <span style={{ fontSize: 10, marginLeft: 4, color, fontWeight: 400 }}>{extra}</span>
                    </p>
        ...
        {!health && <p style={{ margin: 0, fontSize: 13, color: 'var(--text-tertiary)' }}>Loading system health...</p>}
      </div>
    </div>
  );
}
