        ...
        <p style={{ margin: '0 0 4px', fontWeight: 600, color: 'var(--text-secondary)' }}>{label}</p>
        {
          ...
          payload.map((p: any) => (
            <p key={p.name} style={{ margin: 0, color: p.color }}>
              {p.name}:{' '}
              {typeof p.value === 'number' && p.name.toLowerCase().includes('$') ? formatCurrency(p.value) : p.value}
            </p>
          ))
        }
      </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          {/* Date range selector */}
          <div
            style={{ display: 'flex', borderRadius: 6, border: '1px solid var(--border-default)', overflow: 'hidden' }}
          >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 16 }}>
        <StatCard
          ...
          value={stats ? stats.held : loading ? '...' : '\u2014'}
          icon={ShieldAlert}
          accent="red"
        />
        <StatCard
          label="Approved today"
          value={stats ? stats.approvedToday : loading ? '...' : '\u2014'}
          icon={CheckCircle}
          accent="green"
        />
        <StatCard
          label="Total Intercepted"
          value={stats ? stats.interceptedTotal : loading ? '...' : '\u2014'}
          icon={Activity}
          accent="blue"
        />
        <StatCard
          label="Total Value Pended"
          value={stats?.totalValueHeld ? formatCurrency(stats.totalValueHeld) : loading ? '...' : '$0'}
          icon={DollarSign}
          accent="amber"
        />
        <StatCard
          label="Model status"
          value={stats ? stats.modelStatus : loading ? '...' : '\u2014'}
          icon={Cpu}
          accent={stats?.modelStatus === 'healthy' ? 'green' : 'red'}
        />
        <StatCard
          label="F1 Score"
          value={
            typeof modelPerf?.f1 === 'number'
            ...
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={riskData} layout="vertical">
                ...
                <Bar dataKey="count" radius={[0, 4, 4, 0]}>
                  {riskData.map((entry: { level: string; count: number }) => (
      ...
      <section className="card" style={{ padding: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          {!allHeld.length && !heldLoading && (
            ...
            >
              No claims on hold. Click{' '}
              <span style={{ color: 'var(--accent-primary)', fontWeight: 700 }}>Seed Sample Data</span> in the top bar
              to load FHIR sample bundles.
            </div>
          )}
        </div>
      </section>
