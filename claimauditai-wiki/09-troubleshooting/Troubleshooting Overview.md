...
| **Claims misclassified or missing in audit ledger** | `GetLedger` evaluated `priority="stat"` before `status`, mislabeling resolved escalated claims; SQL excluded cancelled tasks. | [[Rejected Claims Missing From Ledger]] |
| **Missing custom override notes or stale history details** | `GetLedger` omitted rationale extension; `ClaimDetail.tsx` read first chronological action instead of latest. | [[Rejected Claims Missing From Ledger]] |
...
| **NLP/Embedded Python throws NumPy/SciPy AttributeError** | Dependency conflict between NumPy and SciPy due to uncoordinated pip installations | [[NumPy and SciPy Dependency Conflicts]] |
| **ObjectScript methods evaluate incorrect logical OR** | Legacy non-short-circuiting logical OR operator `!` used instead of `||` | [[ObjectScript Logical Operators]] |
