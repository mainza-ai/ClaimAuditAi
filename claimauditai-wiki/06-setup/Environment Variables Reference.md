...
| :--- | :---: | :--- | :--- |
| `LLM_PROVIDER` | Yes | `nvidia` | Defines the LLM engine: `nvidia` (cloud), `ollama` (local), `openai`, or `openrouter`. |
...
| `OPENROUTER_API_KEY` | Conditional | — | API key for OpenRouter (also checked as `OPEN_ROUTER_API_KEY`). |
| `OPENROUTER_BASE_URL` | No | `https://openrouter.ai/api/v1` | Endpoint URL for the OpenRouter API. |
| `OPENROUTER_MODEL` | No | `google/gemini-2.5-pro` | The model used on the OpenRouter gateway. |
...
## FHIR Customization (Legacy)

These variables are **commented out** in `.env.example` and are not actively used by the current codebase. They are retained for reference in case a future reimplementation uses Python-based claim interception:

| Variable Name | Required | Default Value | Description |
| :--- | :---: | :--- | :--- |
| `FHIR_CUSTOMIZATION_PATH` | No | `/home/irisowner/dev/src/python` | Target path for the Embedded Python modules inside the container. |
...
The actual interception is handled by ObjectScript classes (`ClaimAudit.FHIR.Interactions`, `ClaimAudit.FHIR.InteractionsStrategy`, `ClaimAudit.FHIR.RepoManager`), which extend the built-in `HS.FHIRServer` storage architecture.
