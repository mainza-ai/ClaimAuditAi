...
- **Credential Storage**: User passwords are hashed using PBKDF2-SHA256 (100,000 iterations, random salt) and stored in `^ClaimAuditAI` globals within the INTEROP namespace for all 5 role types (Admin, Director, Specialist, Auditor, Viewer). Legacy HMAC-SHA256 hashes are automatically upgraded to PBKDF2 on the next successful login. No `%SYS` namespace access required for the CSP Gateway's `UnknownUser`.
